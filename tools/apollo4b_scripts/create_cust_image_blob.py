...
sys.path.append(path_div.join('oem_tools_pkg/common_utils'.split('/')))
...
def parse_arguments():
    ...
    parser.add_argument('--load-address', dest='load_address', type=auto_int, default=hex(AM_SECBOOT_DEFAULT_NONSECURE_MAIN),
                        help='Load address of the binary.')

    parser.add_argument('--enc-algo', dest='enc_algo', default=0, type=int, choices=BootImage.enc_algorithms,
                        help='Encryption algorithm to use.')

    parser.add_argument('--auth-algo', dest='auth_algo', default=0, type=int, choices=BootImage.auth_algorithms,
                        help='Authentication algorithm to use.')

    parser.add_argument('--auth-key', dest='auth_key', default=0, type=auto_int,
                        help='Authentication key.')

    parser.add_argument('--kek-index', dest='kek_index', default=0, type=auto_int,
                        help='Key-encryption-key index.')

    parser.add_argument('-t', '--image-type', dest='image_type', type=str, default='firmware', choices=BootImage.image_types,
    ...
    parser.add_argument('--offset', dest='offset', type=auto_int, default=0,
                        help='Offset (for info0 or patch images)')

    parser.add_argument('--certificate', dest='certificate', type=str,
                        help='Certificate file.')

    parser.add_argument('-o', '--output', dest = 'output', default='outimage',
                        help = 'Output filename (without the extension)')

    parser.add_argument('--key-table', dest = 'key_table', default='keys.ini',
...
class BootImage:
    ...
    def add_header(self, binary_array, crc_check=True):
        ...
        # member variables.
        for name, field in self.header_offsets.items():
    ...
    def sign_byte_array_dll(self, B, key, auth_algo=0):
        DLLHandle = ctypes.cdll.LoadLibrary("oem_tools_pkg/lib/libsbu_crypto.so")
